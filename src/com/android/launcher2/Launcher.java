...
public final class Launcher extends Activity
    ...
    private Timer mBoostUPerfTimer  = new Timer();
    private BoostUPerfTimerTask mBoostUPerfTask = null;
    private boolean mBoostUperfEnable = false;
    private boolean mBoostUperfHomeEnter = false;
    private final int BOOSTUPERF_TIMER_PERIOD = 2*1000;
    ...
    class BoostUPerfTimerTask extends TimerTask{
        public void run(){
            Log.d(TAG, " schedule task: " + BOOSTUPERF_TIMER_PERIOD);
            Intent intent = new Intent();
            intent.setAction(Intent.ACTION_BOOST_UP_PERF);
            intent.putExtra("mode", "mode_home_enter");
            sendBroadcast(intent);
            mBoostUperfHomeEnter = true;
        }
    }
    private void startTimer_boostUperf(){
        ...
        if(mBoostUPerfTimer != null && mBoostUperfEnable){
            //cancel the old one
            if(mBoostUPerfTask != null){
                mBoostUPerfTask.cancel();
                mBoostUPerfTask = null;
            }
            mBoostUPerfTask = new BoostUPerfTimerTask();
            mBoostUPerfTimer.schedule(mBoostUPerfTask, BOOSTUPERF_TIMER_PERIOD);
        }
    }
    private void cancelTimer_boostUperf(){
        if(mBoostUPerfTask != null){
            mBoostUPerfTask.cancel();
            mBoostUPerfTask = null;
        }
        if (mBoostUperfHomeEnter) {
            Intent intent = new Intent();
            intent.setAction(Intent.ACTION_BOOST_UP_PERF);
            intent.putExtra("mode", "mode_home_exit");
            sendBroadcast(intent);
            mBoostUperfHomeEnter = false;
        }
    }
...
//    }
    ...
    protected void onResume() {
        ...
        if(mOnResumeState == State.NONE && mState == State.WORKSPACE){
            mBoostUperfEnable = true;
            startTimer_boostUperf();
        }
        ...
        if (tag instanceof ShortcutInfo) {
    		...
    		try {
    	    	...
    	    	case R.id.radio: {
    	    		...
    	    		it.setClassName("com.tw.radio", "com.tw.radio.RadioActivity");
    	    		startActivity(it);
    	    		break;
    	    	}
    	    	case R.id.music: {
    	    		Intent it = new Intent();
    				it.setClassName("com.tw.music", "com.tw.music.MusicActivity");
    	    		startActivity(it);
    	    		break;
    	    	}
    	    	case R.id.bt: {
    	    		Intent it = new Intent();
    	    		it.setClassName("com.tw.bt", "com.tw.bt.ATBluetoothActivity");
    	    		startActivity(it);
    	    		break;
    	    	}
    	    	case R.id.navi: {
    				...
    				try {
    					br = new BufferedReader(new FileReader("/data/tw/navi_name"));
    ...
    private void runNewAppsAnimation(boolean immediate) {
        ...
        Collections.sort(mNewShortcutAnimateViews, new Comparator<View>() {
            ...
            public int compare(View a, View b) {
            	if((a instanceof CellLayout) && (b instanceof CellLayout)) {
...
//        boolean voiceVisible = updateVoiceSearchIcon(searchVisible);
        mSearchDropTargetBar.onSearchPackagesChanged(false, false);
    }
